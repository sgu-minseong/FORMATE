...
아래 문서에 과거 표현이 남아 있더라도, 새 에이전트는 이 섹션을 최우선 사실로 보고 작업하세요.

FORMATE의 현재 포지셔닝은 “오프라인 상담 자리에서 바로 계산”이 아니라 **인테리어 업체가 자기 조건별 단가표를 저장하고, 들어온 문의를 빠르게 견적서로 바꾸는 도구**입니다. 핵심 문장은 “우리 업체 단가표로 빠르게 만드는 인테리어 견적서”입니다.

현재 구현 상태:

- 업체 코드 + 비밀번호 로그인 사용. 정식 회원가입/Auth 아님.
- `companies.admin_password_hash`는 내부 테스트 단계에서 평문 비밀번호로 비교함.
- localStorage key:
...
- 업체 변경 시 localStorage 업체 정보와 sessionStorage 관리자 인증을 모두 지움.
- 모든 핵심 조회/저장은 로그인된 `selectedCompanyId` 기준. demo company id 하드코딩으로 되돌리면 안 됨.
- 관리자 페이지는 바로 진입하지 않음. 현재 업체의 관리자 비밀번호 재확인 후 진입.
- 관리자 인증은 같은 브라우저 세션 안에서만 유지.
- 관리자 페이지 제목/성격은 “조건별 단가표 관리”. 고객용 견적서가 아니라 업체 내부 단가표 설정 화면임.
- 관리자 홈에는 “조건별 단가표 편집”, “세부견적 관리” 중심으로 유지. “견적서 관리”는 관리자 홈에서 제거됨.
- 랜딩의 “저장된 견적서” 카드가 저장 견적 관리 화면(`admin-estimates`)으로 감.
- 새 업체에 기본 시공항목/세부항목이 없으면 FORMATE 기본 카탈로그를 현재 업체 `company_id`로 준비하는 흐름이 있음.
- 기본 카탈로그는 18개 대분류, 190개 세부항목. 단가/인건비는 만들지 않고 0 또는 빈 값에서 업체가 직접 입력.
- 기본 생성은 `construction_items`, `construction_subitems`만 다룸. `admin_condition_templates`, `admin_condition_template_values` 자동 생성 금지.
- 관리자 항목 화면에는 대분류 접기/펼치기, 전체 펼치기/접기, 즐겨찾기만 보기, 검색이 있음.
- 숨김/is_active 기능은 DB 컬럼 확인 전까지 보류. 임의 SQL/스키마 변경 금지.

최근 UX 개선:

- 신규 견적서 만들기에서 평수/신축·구축/확장 여부를 선택하면 현재 업체의 조건별 단가표를 조회해 견적 초안을 만듦.
- 조건별 단가표 값이 있는 항목은 초안에서 자동 포함 상태로 시작.
- 단가표가 없으면 기본 항목 구조를 보여주고 금액은 0/빈 값으로 시작.
- 체크박스는 “이번 견적 포함 여부”만 담당.
- 행 우측 화살표는 “세부 수정 펼치기/닫기”만 담당.
- 펼침 상태 변경이 총액 포함 여부를 바꾸면 안 됨.
- 미리보기/PDF에는 견적서 번호, 작성일, 유효기간 30일, 부가세 표시, 업체명, 고객명/연락처, 현장 주소, 시공 예정일, 견적 조건, 제외 항목이 표시됨.
- PDF 파일명은 `견적서_{업체명}_{고객명또는주소}_{작성일}.pdf` 형식.
- 저장 견적 목록은 현재 업체 `company_id` 기준으로만 조회.
- 저장 견적 목록에 작성일, 고객명, 현장 주소, 시공 예정일, 총액 표시.
- 저장 견적 검색은 고객명 또는 주소 기준.
- 저장 견적 상세에서 다시 열기/PDF 다운로드 가능.
- 저장 견적 “복사해서 새 견적서 만들기” 구현됨. 기존 견적을 덮어쓰지 않고 새 초안으로 열며 저장 시 새 estimate insert.
- 고객명/연락처/부가세/작성일/유효기간/견적서 번호는 DB 컬럼 추가 없이 `items_data.estimateMeta`에 저장.

절대 금지:

- DB 스키마 임의 변경 금지.
- Supabase SQL 임의 작성/실행 금지.
- 정식 회원가입, 이메일 인증, 비밀번호 찾기, 소셜 로그인, 결제, 권한 관리 추가 금지.
- Supabase service role key를 프론트엔드에 노출 금지.
- company_id 분리 제거 금지.
- demo company id fallback으로 되돌리기 금지.
- 기존 견적 작성/저장/PDF, 관리자 조건별 템플릿 구조 파괴 금지.
- 원가, 마진율, 예상 이익, CRM, 알림톡, 링크 열람 추적, 프랜차이즈 기능 추가 금지.

검증 기준:

- `npm run build`는 통과해야 함. 현재 Vite chunk size warning은 기존 경고로 치명 오류 아님.
- 로컬 dev 서버는 `http://127.0.0.1:5173`에서 사용 가능했던 상태.
- 실제 Supabase 데이터 검증은 demo/dad/kim 업체별로 로그인해 company_id 분리, 관리자 인증, 단가표 저장/불러오기, 견적 저장/PDF, 저장 견적 검색/복사를 확인해야 함.

## 1. 서비스 개요

FORMATE는 인테리어 업체가 자기 조건별 단가표를 저장하고, 들어온 문의를 빠르게 견적서로 바꾸는 B2B 견적 관리 프로토타입입니다.
...
- 들어온 문의 조건을 선택하면 조건별 단가표를 불러와 견적 초안을 바로 만들 수 있다.
...
- FORMATE 브랜드와 “우리 업체 단가표로 빠르게 만드는 인테리어 견적서” 메시지
- 오른쪽에 견적서 카드 형태의 정적 preview UI
- 주요 CTA:
  - 신규 견적서 만들기: `condition` 화면으로 이동
  - 조건별 단가표 관리: 관리자 비밀번호 확인 후 `admin` 화면으로 이동
- 보조 CTA:
  - 상담/공사 메모: 현재 준비중/기존 placeholder 유지
  - 저장된 견적서: `admin-estimates` 화면으로 이동
...
- 체크박스는 이번 견적 포함 여부, 우측 화살표는 세부 수정 펼치기/닫기입니다.
...
- 고객명, 연락처, 현장 주소, 시공 예정일, 부가세 표시 입력
- 견적서 번호, 작성일, 유효기간 30일, 견적 조건, 조건 평수, 견적 기준 평수 표시
- 선택 항목 표:
  - 시공 항목
  - 소재
  - 수량
...
- “견적서 저장”:
  ...
  - `items_data`에 선택 항목, 추가금/할인, 현장 메모, 합계, `estimateMeta` 데이터를 저장
- “PDF 다운로드”:
  - `previewPdfRef` 영역을 `html2canvas`로 캡처
  - `jspdf`로 A4 PDF 생성
  - 파일명은 `견적서_{업체명}_{고객명또는주소}_{작성일}.pdf`
...
- 고객명 또는 주소 기준 검색. Supabase에서는 현재 업체 `company_id`로 조회한 뒤 클라이언트에서 검색어 필터링.
