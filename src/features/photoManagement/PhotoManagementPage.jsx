...
export const PHOTO_MANAGEMENT_MODES = [
  ...
  },
  {
    id: "price",
    title: "공사 가격별 사진 관리",
    description: "공사 금액대별 기존 사진 자료를 관리합니다.",
    icon: WalletCards,
  },
];
...
export default function PhotoManagementPage({ controller }) {
  ...
  if (mode === "library") return <PhotoLibraryPlaceholder onBack={() => setMode("")} />;
  return <LegacyPricePhotoManagement controller={controller} onBack={() => setMode("")} />;
}

export function LegacyPricePhotoManagement({ controller, onBack }) {
  const {
    photoTab,
    setPhotoTab,
    photoTypes,
  ...
  return (
    <main className="panel-page photo-management-page">
      <section className="photo-management-panel">
        <header className="photo-management-toolbar">
          <div className="photo-mode-titleline">
            {onBack && <button type="button" onClick={onBack} aria-label="사진 관리 모드 선택으로 돌아가기"><ArrowLeft size={18} /></button>}
            <h1>사진 관리/확인</h1>
          </div>
